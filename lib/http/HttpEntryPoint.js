...
HttpEntryPoint.prototype.fetchFirstUrl = function (date, departureStop) {
    ...
    return jsonld.promises.frame(document, {
      ...
      "@graph" : {
        ...
        "search" : {
          "mapping" : {
            ...
            "property":"http://semweb.mmlab.be/ns/linkedconnections#page"
          }
        }
      }
    }).then(function (data) {
      if (data["@graph"][0] && data["@graph"][0].search && data["@graph"][0].search.template) {
        ...
        if (departureStop) {
          ...
          params[data["@graph"][0].search.mapping[1].variable] = departureStop;
        } else {
          params[data["@graph"][0].search.mapping[0].variable] = date.toISOString();
        }
