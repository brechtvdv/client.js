...
var HttpNeighbouringConnections = function (entrypoint, query, http) {
  ...
  this._bestStops.enq({'stopId': departureStop, 'minimumDepartureTime': departureTime, 'priority': Infinity});
  this._seenStops = { departureStop: true };
  this._toBeProcessedByCSA = 0; // When all connections are scanned by CSA, fetch next page
...
HttpNeighbouringConnections.prototype.updateProcessed = function () {
  this._toBeProcessedByCSA--;
};
...
HttpNeighbouringConnections.prototype.analyse = function (connection) {
  debugger;
  // What if departure stop get's accessed earlier suddenly?
  if (!this._seenStops[connection['arrivalStop']]) {
    this._seenStops[connection['arrivalStop']] = true;
    ...
    var length = this._haversineDistance(coordsDepartureStop, coordsArrivalStop);
    var time = (new Date(connection['arrivalTime']) - new Date(connection['departureTime']))/1000/60; // time difference in minutes
    ...
    // calculate priority
    var priority = this._calculatePriority(length, time, restDistanceToGoal, cos, countDirectStopsArrivalStop);
...
/*
 * @param distance Distance the connection travels
 * @param time How long the connection takes
 * @param cos Cosine simularity of connectionvector and direct way vector
 * @param distanceToGoal
 * @param importance amount of direct stops
 */
HttpNeighbouringConnections.prototype._calculatePriority = function (length, time, restDistanceToGoal, cos, importance) {
  var alpha = 1;
  var beta = 2;
  var gamma = 3;
  var kappa = 3;
  return (alpha*length/time + beta*cos + gamma*importance + kappa*restDistanceToGoal);
};
...
HttpNeighbouringConnections.prototype._read = function () {
  ...
  if (this._connections.length === 0) {
    // We can only make a good next stop when all previous connections have been processed by CSA
    if (this._toBeProcessedByCSA === 0) {
      var self = this;
      ...
      this._entry.fetchFirstUrl(nextStop.minimumDepartureTime, nextStop.stopId).then(function (url) {
        self._fetchNextPage(url).then(function (connections) {
          self._toBeProcessedByCSA = connections.length;
          if (connections.length === 0) {
            // Choose another stop
            if (self._seenStops.length > 0) {
              self._read(); // read another stop
            } else {
              console.error('end of the stream: no next stop');
              self.push(null);
            }
          } else {
