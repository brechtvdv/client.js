...
var HttpNeighbouringConnections = function (entrypoint, query, http) {
  ...
  this._decreasePriority = 0.1;
  this._interval = 10; // When retrying a stop, add this interval to new departure time (minutes)
};
...
HttpNeighbouringConnections.prototype._getNextStop = function () {
  if (this._bestStops.size() > 0) {
    ...
    this._bestStops.enq({'stopId': stop.stopId, 'minimumDepartureTime': new Date(stop.minimumDepartureTime.getTime() + this._interval*60000), 'priority': stop.priority - this._decreasePriority});
