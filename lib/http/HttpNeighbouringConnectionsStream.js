...
var HttpNeighbouringConnections = function (entrypoint, query, http) {
  ...
  this._query = query;
  var departureStop = this._query.departureStop;
  var departureTime = this._query.departureTime;
  ...
  this._timeOffset = 120; // match this with server
  this._totalTimeOffSet = 120;
  this._total = 0; // When 0 stops, rechoose next stop
};
...
HttpNeighbouringConnections.prototype._getNextStop = function () {
  if (this._bestStops.size() == 0) {
    // Add departure stop again with later departure time
    var departureStop = this._query.departureStop;
    var departureTime = new Date(this._query.departureTime.getTime() + this._totalTimeOffSet*60000);
    this._totalTimeOffSet += this._timeOffset;
    this._bestStops.enq({'stopId': departureStop, 'minimumDepartureTime': departureTime, 'priority': 3});
  }
  if (this._bestStops.size() > 0 && this._total == 0) {
...
HttpNeighbouringConnections.prototype.update = function () {
  this._total--;
  if (this._total == 0) {
    this._read();
  }
      ...
      this._entry.fetchFirstUrl(nextStop.minimumDepartureTime, nextStop.stopId).then(function (url) {
        self._fetchNextPage(url).then(function (connections) {
          if (connections.length === 0) {
            ...
            self._total = connections.length;
