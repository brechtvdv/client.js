...
/*
 * Calculates the importance of the departure and arrival stop if not already seen
 */
HttpNeighbouringConnections.prototype._analyse = function (connections) {
  for (var i = 0; i < connections.length; i++) {
    var connection = connections[i];
    // departure stop of connection can't be better
...
HttpNeighbouringConnections.prototype._read = function () {
  if (this._connections.length === 0) {
    ...
    var self = this;
    // Decision: which departure stop is best
    ...
    this._entry.fetchFirstUrl(nextStop.minimumDepartureTime, nextStop.stopId).then(function (url) {
      self._fetchNextPage(url).then(function (connections) {
        if (connections.length === 0) {
          // Choose another stop if there are other unseen
          ...
          self.pause(); // So it can analyse the stops first
          self._analyse(connections);
        }
      });
    });
  } else {
